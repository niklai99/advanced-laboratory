...
def read_data(data_path, file_name, data_format):
    with uproot.open(f"{data_path}/{file_name}") as data_file:
        ...
        energy_short = data_file[data_format.to_energy_short].array().to_numpy()
    return pd.DataFrame({"board": boards, "channel": channels, "flag": flags, "timestamp": time, "energy": energy, "energy_short": energy_short})